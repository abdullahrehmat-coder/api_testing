...
def run_graphql(query, variables, headers=None):
    ...
    try:
        resp = requests.post(BASE_URL, json={"query": query, "variables": variables},
                            headers=headers or HEADERS, timeout=10)
...
def test_06_boundary_invalid_id():
    ...
    resp, _, _ = run_graphql(GRAPHQL_QUERY, {"systemId": "invalid-id-999", "start": TEST_VARS["start"], "end": TEST_VARS["end"]})
    has_error = resp and ("errors" in resp.json() or resp.status_code != 200)
    log_result("Boundary: Invalid ID", "Functional Testing", has_error, "Error returned" if has_error else "Should error", {})
    assert has_error
...
def test_10_authentication_integration():
    ...
    passed = resp and resp.status_code != 200
    log_result("Authentication Integration", "Integration Testing", passed, "Invalid token rejected", {})
    assert passed
...
def test_16_authentication_required():
    ...
    passed = resp and resp.status_code in [401, 403]
    log_result("Authentication Required", "Security Testing", passed, "Unauthenticated request rejected", {})
    assert passed
...
def test_17_token_validation():
    ...
    passed = resp and resp.status_code in [401, 403]
    log_result("Token Format Validation", "Security Testing", passed, "Malformed token rejected", {})
    assert passed

def test_18_sql_injection():
    """SECURITY: SQL injection prevention"""
    mal_vars = {"systemId": "'; DROP TABLE--", "start": TEST_VARS["start"], "end": TEST_VARS["end"]}
    resp, _, _ = run_graphql(GRAPHQL_QUERY, mal_vars)
    passed = resp and ("errors" in resp.json() or resp.status_code != 200)
    log_result("SQL Injection Prevention", "Security Testing", passed, "Injection safely handled", {})
    assert passed
